...
Большинство людей привыкли, что Chromium - это браузер и основа для других браузеров. Но на самом деле его компоненты и систему сборки можно использовать для создания своих кроссплатформенных приложений.
...
Для начала надо создать папку, где будут лежать исходники и перейти в неё. Например в домашней директории:
```bash
...
В качестве первого примера использования кодовой базы `Chromium` в качестве фреймворка предлагаю "поиграться" с командной строкой.
...
Все аргументы начинающиеся с символа `-` возвращаются в виде `base::SwitchMap` (по сути `map<string, string>`) с помощью метода `GetSwitches`. Все остальные аргументы возвращаются в виде `base::StringVector` (по сути `vector<string>)`. Этих знаний достаточно, чтобы реализовать код для задачи:
```c++
...
Упрощённая (и немного устаревшая) схема выглядит так:
...
Чтобы послать запрос, надо создать объект `URLRequest` с помощью метода `CreateRequest` объекта `ctx`.
